namespace WallpaperFetch
{
    class Profile
    {
        ...
        public Profile()
        {
            var profileFolder = Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
                "WallpaperFetch");
            var historyFolder = Path.Join(profileFolder, "history");
            var profileApp = Path.Join(profileFolder, "application.exe");
            var settingsFile = Path.Join(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile),
                ".wallpaper.json");
            var schemaFile = Path.Join(profileFolder,
                "schema.json");
        ...
        public async Task UpdateOrCreateProfile()
        {
